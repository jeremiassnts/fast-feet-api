...
export class CreateRecipientUseCase {
    constructor(private recipientsRepository: RecipientsRepository, private usersRepository: UsersRepository) { }
    async execute({ address, adminId, latitude, longitude, name, email }: CreateRecipientUseCaseRequest) {
        ...
        const newRecipient = await this.recipientsRepository.create(new Recipient({
            name,
            email,
            address,
            latitude,
